...
public class VentanaPrincipal extends javax.swing.JFrame {
    ...
    private boolean audioPausado = false;
    SMClipPlayer player = null;
    SMSoundRecorder recorder = null;
    ImageIcon pausa = new ImageIcon(getClass().getResource("/iconos/pausa.png"));
    ImageIcon play = new ImageIcon(getClass().getResource("/iconos/play.png"));
    ...
    private final ManejadorAudio manejadorAudio = new ManejadorAudio();
    ...
    public VentanaPrincipal() {
        ...
        botonMenuAbrirAudio.addActionListener(manejadorMenu);
        ...
        botonAbrirAudio.addActionListener(manejadorMenu);
        ...
        botonPlay.addActionListener(manejadorAudio);
        botonStop.addActionListener(manejadorAudio);
        botonGrabar.addActionListener(manejadorAudio);
        listaReproduccion.addActionListener(manejadorAudio);
        ...
        public void actionPerformed(ActionEvent ae) {
            if (ae.getSource() == botonMenuNuevo || ae.getSource() == botonNuevo) {
                ...
                FileNameExtensionFilter imageFilter = new FileNameExtensionFilter(
                        "Imágenes: " + Arrays.toString(ImageIO.getReaderFormatNames()),
                        ImageIO.getReaderFormatNames()
                );
                FileNameExtensionFilter audioFilter = new FileNameExtensionFilter(
                        "Archivos de audio (WAV, AU, AIFF)", "wav", "au", "aiff"
                );
                dlg.addChoosableFileFilter(imageFilter);
                dlg.addChoosableFileFilter(audioFilter);
                ...
                if (resp == JFileChooser.APPROVE_OPTION) {
                    try {
                        ...
                        if (Arrays.asList(ImageIO.getReaderFormatNames()).contains(extension)) {
                            ...
                            vi.setVisible(true);
                        } else if (extension.equals("wav") || extension.equals("aiff") || extension.equals("au")) {
                            FiletoString archivoAudio = new FiletoString(dlg.getSelectedFile().getAbsolutePath());

                            String fileName = archivoAudio.getName().toLowerCase();
                            if (!(fileName.endsWith(".wav") || fileName.endsWith(".au") || fileName.endsWith(".aiff"))) {
                                throw new IllegalArgumentException("Formato de archivo no soportado");
                            }

                            listaReproduccion.addItem(archivoAudio);
                            listaReproduccion.setSelectedItem(archivoAudio);
                        } else {
                            throw new IllegalArgumentException("Formato de archivo no soportado");
                        }
                    } catch (Exception ex) {
            ...
            } else if (ae.getSource() == botonMenuAbrirAudio || ae.getSource() == botonAbrirAudio) {
                JFileChooser dlg = new JFileChooser();
                dlg.setFileFilter(new FileNameExtensionFilter("Archivos de audio (WAV, AU, AIFF)", "wav", "au", "aiff"));
                int resp = dlg.showOpenDialog(null);
                if (resp == JFileChooser.APPROVE_OPTION) {
                    try {
                        FiletoString archivoAudio = new FiletoString(dlg.getSelectedFile().getAbsolutePath());
                        ...
                        JOptionPane.showMessageDialog(null, "Error al abrir el archivo de audio: " + ex.getMessage(),
                                "Error", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
    ...
    /**
     * Clase que se encarga de gestionar las acciones de reproducción, grabación
     * y selección de archivos de audio, incluyendo el manejo de eventos para
     * los botones y temporizadores.
     */
    class ManejadorAudio implements ActionListener, LineListener {

        private File archivoSalida;
        boolean modoControl = false;
        private Timer timerReproduccion;
        private Timer timerGrabacion;
        private int segundosReproduccion = 0;
        private int segundosGrabacion = 0;

        /**
         * Constructor que inicializa los temporizadores para reproducción y
         * grabación.
         */
        public ManejadorAudio() {
            timerReproduccion = new Timer(1000, e -> {
                segundosReproduccion++;
                int minutos = segundosReproduccion / 60;
                int segundos = segundosReproduccion % 60;
                tiempoReproduccion.setText(String.format("%02d:%02d", minutos, segundos));
            });
            timerGrabacion = new Timer(1000, e -> {
                segundosGrabacion++;
                int minutos = segundosGrabacion / 60;
                int segundos = segundosGrabacion % 60;
                temporizadorGrabacion.setText(String.format("%02d:%02d", minutos, segundos));
            });
        }

        /**
         * Maneja las acciones de los botones y la selección de archivos.
         *
         * @param ae El evento que desencadenó la acción.
         */
        @Override
        public void actionPerformed(ActionEvent ae) {
            if (ae.getSource() == botonPlay) {
                if ("play".equals(botonPlay.getActionCommand())) {
                    ...
                    if (f != null) {
                        try {
                            if (player == null) {
                                ...
                                tiempoReproduccion.setText("00:00");
                            }
                            audioPausado = false;
                            player.play();
                            botonPlay.setIcon(pausa);
                            botonPlay.setActionCommand("pause");
                            timerReproduccion.start();
                        } catch (Exception ex) {
                            JOptionPane.showMessageDialog(null,
                                    "Error al reproducir el archivo: " + ex.getMessage(),
                                    "Error",
                                    JOptionPane.ERROR_MESSAGE);
                        }
                    }
            ...
            } else if (ae.getSource() == botonStop) {
                if (player != null) {
                    player.stop();
                    player = null;
                    timerReproduccion.stop();
                ...
                if (recorder != null) {
                    ...
                    recorder = null;
                    timerGrabacion.stop();
                    temporizadorGrabacion.setText("00:00");

                    if (!modoControl) {
                        JFileChooser dlgGuardar = new JFileChooser();
                        dlgGuardar.setFileFilter(new FileNameExtensionFilter("Archivos de audio (WAV, AU, AIFF)", "wav", "au", "aiff"));
                        int resp = dlgGuardar.showSaveDialog(null);
                        if (resp == JFileChooser.APPROVE_OPTION) {
                            try {
                                File archivoSeleccionado = dlgGuardar.getSelectedFile();
                                String nombreArchivo = archivoSeleccionado.getName().toLowerCase();
                                if (!(nombreArchivo.endsWith(".wav") || nombreArchivo.endsWith(".au") || nombreArchivo.endsWith(".aiff"))) {
                                    throw new IllegalArgumentException("Solo se permiten formatos .wav, .au, .aiff");
                                }
                                if (!archivoSalida.renameTo(archivoSeleccionado)) {
                                    JOptionPane.showMessageDialog(null,
                                ...
                                FiletoString archivoFinal = new FiletoString(archivoSeleccionado.getAbsolutePath());
                                listaReproduccion.addItem(archivoFinal);
                            } catch (Exception ex) {
                                JOptionPane.showMessageDialog(null,
                                        "Error al guardar el archivo de audio: " + ex.getMessage(),
                                        "Error",
                                        JOptionPane.ERROR_MESSAGE);
                            }
                        }
            ...
            } else if (ae.getSource() == botonGrabar) {
                modoControl = (ae.getModifiers() & ActionEvent.CTRL_MASK) != 0;
                ...
                if (modoControl) {
                    ...
                    int resp = dlgGuardar.showSaveDialog(null);
                    if (resp == JFileChooser.APPROVE_OPTION) {
                        archivoSalida = dlgGuardar.getSelectedFile();
                        String nombreArchivo = archivoSalida.getName().toLowerCase();

                        if (!(nombreArchivo.endsWith(".wav") || nombreArchivo.endsWith(".au") || nombreArchivo.endsWith(".aiff"))) {
                            JOptionPane.showMessageDialog(null,
                                    "Formato no permitido. Solo se permiten formatos .wav, .au, .aiff",
                                    "Error",
                                    JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                        FiletoString archivoFinal = new FiletoString(archivoSalida.getAbsolutePath());
                        listaReproduccion.addItem(archivoFinal);
                    } else {
                        return;
                    }
                } else {
                    archivoSalida = new File("temp_grabacion.wav");
                }

                if (archivoSalida != null) {
                    recorder = new SMSoundRecorder(archivoSalida);
            ...
            } else if (ae.getSource() == listaReproduccion) {
                if (player != null) {
                    player.stop();
                    player = null;
                    botonPlay.setIcon(play);
                ...
                File seleccionado = (File) listaReproduccion.getSelectedItem();
                if (seleccionado != null && seleccionado.exists()) {
                    try {
                        AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(seleccionado);
                        AudioFormat format = audioInputStream.getFormat();
                        long frames = audioInputStream.getFrameLength();
                        double durationInSeconds = (frames + 0.0) / format.getFrameRate();

                        int minutos = (int) (durationInSeconds / 60);
                        int segundos = (int) (durationInSeconds % 60);

                        duracionAudio.setText(String.format("%02d:%02d", minutos, segundos));
                    } catch (Exception e) {
                        duracionAudio.setText("00:00");
                    }
                } else {
                    duracionAudio.setText("00:00");
                }
        ...
        /**
         * Responde a los eventos de línea, como inicio y detención de
         * grabación/reproducción.
         *
         * @param event El evento de línea.
         */
        @Override
    ...
    /**
     * Clase que extiende la clase File para personalizar la representación de
     * los objetos de archivo en formato de texto.
     */
    class FiletoString extends File {

        /**
         * Constructor que inicializa un archivo con la ruta especificada.
         *
         * @param pathname Ruta del archivo.
         */
        public FiletoString(String pathname) {
            super(pathname);
        }

        /**
         * Representación en texto del nombre del archivo.
         *
    ...
    private void initComponents() {
        ...
        botonAbrirAudio = new javax.swing.JButton();
        ...
        tiempoReproduccion = new javax.swing.JLabel();
        ...
        temporizadorGrabacion = new javax.swing.JLabel();
        ...
        botonMenuAbrirAudio = new javax.swing.JMenuItem();
        ...
        botonAbrirAudio.setIcon(new javax.swing.ImageIcon(getClass().getResource("/iconos/openAudio.png"))); // NOI18N
        botonAbrirAudio.setFocusable(false);
        botonAbrirAudio.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        botonAbrirAudio.setVerticalTextPosition(javax.swing.SwingConstants.BOTTOM);
        barraHerramientas.add(botonAbrirAudio);
        ...
        separador8.setBackground(new java.awt.Color(0, 0, 0));
        barraHerramientas.add(separador8);
        ...
        tiempoReproduccion.setText("00:00");
        barraHerramientas.add(tiempoReproduccion);
        ...
        listaReproduccion.setMaximumSize(new java.awt.Dimension(100, 25));
        listaReproduccion.setMinimumSize(new java.awt.Dimension(100, 25));
        listaReproduccion.setPreferredSize(new java.awt.Dimension(100, 25));
        barraHerramientas.add(listaReproduccion);

        duracionAudio.setText("00:00");
        barraHerramientas.add(duracionAudio);
        ...
        temporizadorGrabacion.setText("00:00");
        barraHerramientas.add(temporizadorGrabacion);
        ...
        botonMenuNuevo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/iconos/nuevo.png"))); // NOI18N
        ...
        botonMenuAbrir.setIcon(new javax.swing.ImageIcon(getClass().getResource("/iconos/abrir.png"))); // NOI18N
        ...
        botonMenuGuardar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/iconos/guardar.png"))); // NOI18N
        ...
        botonMenuDuplicar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/iconos/duplicar.png"))); // NOI18N
        botonMenuDuplicar.setText("Duplicar");
        menuArchivo.add(botonMenuDuplicar);

        botonMenuAbrirAudio.setAccelerator(javax.swing.KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_U, java.awt.event.InputEvent.CTRL_DOWN_MASK));
        botonMenuAbrirAudio.setIcon(new javax.swing.ImageIcon(getClass().getResource("/iconos/openAudio.png"))); // NOI18N
        botonMenuAbrirAudio.setText("Abrir audio");
        menuArchivo.add(botonMenuAbrirAudio);
    ...
    private javax.swing.JButton botonAbrirAudio;
    ...
    private javax.swing.JMenuItem botonMenuAbrirAudio;
    ...
    private javax.swing.JLabel temporizadorGrabacion;
    private javax.swing.JLabel tiempoReproduccion;
